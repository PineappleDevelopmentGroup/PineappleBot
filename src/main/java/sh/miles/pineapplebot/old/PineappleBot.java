...
import sh.miles.pineapplebot.old.command.CommandHandler;
import sh.miles.pineapplebot.old.command.text.TextCommandHandler;
import sh.miles.pineapplebot.old.json.JsonConfig;
import sh.miles.pineapplebot.old.json.JsonConfigSection;
import sh.miles.pineapplebot.old.listener.ButtonListener;
import sh.miles.pineapplebot.old.listener.ModalListener;
...
public class PineappleBot {
    ...
    public PineappleBot(JsonConfig config) {
        ...
        this.guild.upsertCommand(Commands.slash("textcommand", "creates a text based command")
                .addOption(OptionType.STRING, "command", "the command name")
                .addOption(OptionType.STRING, "value", "the return value of the command")).queue();
    }
    ...
    public void createCommandEmbed(String command, long messageId) {
        this.embedManager.registerCommandEmbed(messageId, command);
    }

    public void handleEmbedCommand(String command, TextChannel channel) {
        this.embedManager.handleTextCommand(command, channel);
    }
