...
import static sh.miles.pineapplebot.old.structure.Buttons.*;
...
public class EmbedCreator {
    ...
    public Modal handleButton(String button) {
        return switch (button) {
            ...
            case "page-attach-command" -> {
                this.createChannel.editMessageById(this.messageId, "Embed attached to command.").queue();
                yield Modals.CREATOR_ATTACH_COMMAND.getModal();
            }
            default -> null;
        };
    }
    ...
    public void handleModal(String modalId, ModalMapping... mappings) {
        switch (modalId) {
            ...
            case "creator-attach-command" -> PineappleBot.getInstance().createCommandEmbed(mappings[0].getAsString(), this.messageId);
        }
        updateEmbed();
    }

    private void addButtons() {
        this.createChannel.editMessageComponentsById(messageId, firstRow, secondRow, thirdRow, ActionRow.of(SEND, PUBLISH_CHANNEL, ATTACH_COMMAND)).queue();
    }
